...
export function PwaUpdateToast({ show, onReload }: PwaUpdateToastProps) {
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="fixed inset-x-0 bottom-[max(1rem,env(safe-area-inset-bottom))] z-50 mx-auto flex w-[calc(100%-2rem)] max-w-sm items-center gap-3 rounded-2xl border border-warm-border bg-warm-bg-dark-muted px-4 py-3 text-warm-text-on-dark shadow-lg"
    >
