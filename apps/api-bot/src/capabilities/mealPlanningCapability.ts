...
function parseAsk(messageText: string) {
  ...
  return { recipeListName: match[1]!.trim(), shoppingListName: match[2]!.trim(), constraint: match[3]?.trim() || null };
}

function findNamedList(lists: ListSummary[], name: string) {
  const normalized = name.trim().toLowerCase();
  return lists.find((list) => list.name.trim().toLowerCase() === normalized) ?? null;
}
...
function recipesFromList(list: MealPlanningList) {
  ...
  return list.activeItems.map((item) => ({
    ...
    mealType: mealType ? (valueForProperty(item, mealType.id)?.selectOptionId ?? null) : null
  }));
}

function formatPlan(plan: NonNullable<ReturnType<typeof buildWeeklyMealPlan>>, recipeListName: string, shoppingListName: string) {
  return [
    `Weekly meal plan from ${recipeListName}:`,
    ...plan.days.map((day) => `- ${day.weekday}: ${day.dinnerRecipeTitle}`),
    '',
...
export function createMealPlanningCapability({
  ...
  return async (request) => {
    ...
    if (!ask) {
      return { kind: 'reply', text: 'Try /meals Recipes | Shopping, with an optional third part for a one-off constraint.' };
    }
    ...
    if (!plan) return { kind: 'reply', text: 'I need at least one active recipe with an Ingredients text property before I can plan the week.' };

    return { kind: 'reply', text: formatPlan(plan, recipes.name, shopping.name) };
  };
}

const addressableListsForBot = makeFunctionReference<'query', { serviceToken: string; clerkUserId: string }, ListSummary[]>(
...
const currentWeekForBot = makeFunctionReference<'query', { serviceToken: string }, { events: Array<{ start: number }> }>(
...
export function createConvexMealPlanningCapability(config: BotConfig): CapabilityHandler {
  ...
  return createMealPlanningCapability({
    ...
    loadBusyWeekdays: async () => busyWeekdaysFromEvents((await client.query(currentWeekForBot, { serviceToken: config.botServiceToken })).events)
  });
}
