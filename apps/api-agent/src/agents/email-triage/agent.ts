...
relevantThrough is the last calendar date on which any part of a notice remains useful. Return evidence and confidence for that date. When the date is absent or weakly supported, return an empty relevantThrough with low confidence and empty evidence.

A supersession may target only one supplied active notice candidate. Set high confidence only for a clear update, replacement, or cancellation. For uncertain relationships, return an empty noticeId with low confidence and empty evidence. For noNotice, return empty lifecycle targets.
