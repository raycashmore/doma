...
import { weeklyMealsOutcomeSchema } from './schemas.js';
import type { createWeeklyMealsTools } from './tools.js';

export const WEEKLY_MEALS_PROMPT_VERSION = 'weekly-meals-v1';
...
If no slots are open or the available recipes cannot safely cover them, return cannotPropose with a concise explanation. Otherwise return a proposal with one short grounded reason per assignment.`;
...
export function createWeeklyMealsAgent({
  ...
  return new ToolLoopAgent({
    ...
    output: Output.object({
      ...
      schema: weeklyMealsOutcomeSchema
    }),
