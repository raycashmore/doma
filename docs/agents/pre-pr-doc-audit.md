...
2. Search existing docs for the changed terms with `rg` before adding new docs.
3. Update docs close to the source of truth instead of duplicating long explanations.
4. Keep private finance data, bot tokens, Telegram IDs, chat IDs, and real institution/account labels out of examples.
5. Include docs-only verification in the PR notes when the audit finds no doc changes are needed.
